...
4. Always include artifacts as an array of { path, content, language } objects.`;
  ...
  private async callLLM(
    ...
    return this.mockLLMResponse(messages, tools);
  }
  ...
  private async mockLLMResponse(
    ...
    if (tools.includes("code_interpreter")) {
      return {
        artifacts: [
          { path: "src/index.ts", content: "// Generated by mock LLM\nexport const value = 42;", language: "typescript" },
        ],
        summary: "Implemented the requested feature with TypeScript.",
      };
    }
    if (tools.includes("web_search")) {
      return {
        plan: ["Research best practices", "Design API schema", "Implement core logic", "Add tests"],
        summary: "Structured plan generated from web research.",
      };
    }
    if (tools.includes("http_request")) {
      return {
        config: { vercel: { regions: ["iad1", "sfo1"] }, docker: { baseImage: "node:20-alpine" } },
        summary: "Deployment configuration prepared.",
      };
    }
