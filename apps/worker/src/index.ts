...
4. Always include artifacts as an array of { path, content, language } objects.`;
  ...
  private async callLLM(
    ...
    const providers = [
      { url: "https://ai.gateway.lovable.dev/v1/chat/completions", key: env.LOVABLE_API_KEY, model: "google/gemini-3-flash-preview", headers: { "Lovable-API-Key": env.LOVABLE_API_KEY } },
      { url: "https://api.groq.com/openai/v1/chat/completions", key: env.GROQ_API_KEY, model: "llama-3.3-70b-versatile", headers: {} },
      { url: "https://integrate.api.nvidia.com/v1/chat/completions", key: env.NVIDIA_API_KEY, model: "nvidia/llama-3.3-nemotron-super-49b-v1", headers: {} },
    ];
  ...
  private getEnv() {
    return {
      LOVABLE_API_KEY: (globalThis as unknown as Record<string, string | undefined>).LOVABLE_API_KEY,
      GROQ_API_KEY: (globalThis as unknown as Record<string, string | undefined>).GROQ_API_KEY,
      NVIDIA_API_KEY: (globalThis as unknown as Record<string, string | undefined>).NVIDIA_API_KEY,
    };
  }

  private async persistEvent(runId: string, event: AgentEvent): Promise<void> {
    try {
      const env = this.getSupabaseEnv();
      await fetch(`${env.SUPABASE_URL}/rest/v1/run_events`, {
        method: "POST",
        headers: {
  ...
  private getSupabaseEnv() {
    return {
      SUPABASE_URL: (globalThis as unknown as Record<string, string>).SUPABASE_URL ?? "",
      SUPABASE_SERVICE_ROLE_KEY: (globalThis as unknown as Record<string, string>).SUPABASE_SERVICE_ROLE_KEY ?? "",
    };
  }
}
...
class HttpRequestTool implements ToolDefinition {
  ...
  async execute(input: unknown): Promise<ToolResult> {
    ...
    try {
      ...
      return { success: true, output: { status: resp.status, headers: Object.fromEntries(resp.headers), body: text }, durationMs: Date.now() - start };
    } catch (err) {
